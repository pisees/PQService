...
namespace QuickService.Common.ErrorHandling
{
    ...
    public class RetryPolicy
    {
        ...
        /// <summary>
        /// The default retry policy using the <see cref="FixedWaitingPolicy"/> and detecting all exceptions as transient errors (except OperationCanceledException)
        /// </summary>
        public static readonly RetryPolicy DefaultFixed = new RetryPolicy(DefaultMaxAttempts, FixedWaitingPolicy.Default, AllErrorsTransientDetectionStrategy.Instance);
        ...
        /// <summary>
        /// Gets the waiting policy instance.
        /// </summary>
        public readonly WaitingPolicy WaitingPolicy;

        /// <summary>
        /// Gets the error detection strategy.
        /// </summary>
        public ITransientErrorDetectionStrategy DefaultErrorDetectionStrategy { get; private set; }

        /// <summary>
        /// Gets a flag indicating if we should use fast retries for testing.
        /// </summary>
        public bool UseFastRetriesForTesting { get; private set; }
    ...
    public class RetryPolicy<TResult> : RetryPolicy
    {
        #region Construction
        ...
        /// If the <paramref name="maxAttempts"/> argument is not positive.
        /// </exception>
        public RetryPolicy(int maxAttempts, WaitingPolicy waitingPolicy, ITransientErrorDetectionStrategy<TResult> defaultErrorDetectionStrategy, bool useFastRetriesForTesting = false)
            : base(maxAttempts, waitingPolicy, defaultErrorDetectionStrategy, useFastRetriesForTesting)
        {
            // Check the pre-conditions
        ...
        /// </summary>
        public new ITransientErrorDetectionStrategy<TResult> DefaultErrorDetectionStrategy { get; private set; }
        ...
        /// <summary>
        /// Executes an operation that returns a value with retries.
        /// </summary>
        /// <param name="operationToAttempt">The operation, cannot be null.</param>
        /// <param name="errorDetectionStrategy">The error detection strategy to use instead of the default one.</param>
        /// <param name="notificationPolicy">The notification policy to use.</param>
        ...
        /// <exception cref="ArgumentNullException">If some of the non-nullable arguments are null.</exception>
        /// <exception cref="Exception">Any exception thrown by the <paramref name="operationToAttempt"/> after all the attempts have been exhausted.</exception>
        public TResult ExecuteWithRetries(
            Func<TResult> operationToAttempt, 
            ITransientErrorDetectionStrategy<TResult> errorDetectionStrategy = null, 
            IRetryNotificationPolicy<TResult> notificationPolicy = null)
        {
            // Check the pre-conditions
            Guard.ArgumentNotNull(operationToAttempt, nameof(operationToAttempt));
            ...
            for (var attemptCount = 0; attemptCount < MaxAttempts; ++attemptCount)
            {
                ...
                {
                    // Execute the operation
                    var result = operationToAttempt();

                    // Check the result
                    if (errorDetectionStrategy.IsFailure(result))
                    {
                        // Check if it is transient
                        bool isTransient = errorDetectionStrategy.IsTransientFailure(result);
                        bool shouldRetry = ShouldRetry(attemptCount, isTransient);
                        // Notify the caller if we are requested to do so.
                        if (notificationPolicy != null)
                        {
                            notificationPolicy.OnErrorCondition(shouldRetry, result);
                        }
        ...
        /// <summary>
        /// Executes an async operation that returns a value with retries.
        /// </summary>
        /// <param name="operationToAttempt">The operation, cannot be null.</param>
        /// <param name="timeout">Optional timeout for the operation for single retry iteration, can be TimeSpan.Zero or Timeout.Infinite which means no timeout</param>
        /// <param name="cancellationToken">Optional token to cancel the waiting policy, operation and notification.</param>
        ...
        /// <exception cref="OperationCanceledException">If the operation was cancelled.</exception>
        /// <exception cref="Exception">Any exception thrown by the <paramref name="operationToAttempt"/> after all the attempts have been exhausted.</exception>
        public async Task<TResult> ExecuteWithRetriesAsync(
            Func<CancellationToken, Task<TResult>> operationToAttempt,
            TimeSpan timeout = default(TimeSpan),
            CancellationToken cancellationToken = default(CancellationToken),
            ITransientErrorDetectionStrategy<TResult> errorDetectionStrategy = null,
            IRetryNotificationPolicyAsync<TResult> notificationPolicy = null)
        {
            // Check the pre-conditions
            Guard.ArgumentNotNull(operationToAttempt, nameof(operationToAttempt));
            Guard.ArgumentNotNegativeValue(timeout.Ticks, nameof(timeout));
            ...
            for (var attemptCount = 0; attemptCount < MaxAttempts; ++attemptCount)
            {
                ...
                // Construct timeout and cancellation linked token - see comments above (in the base class) 
                // for disposing the timeout cancellation (token) source
                using (var timeoutCts = !IsNoTimeout(timeout) ? new CancellationTokenSource(timeout) : null)
                {
                    using (var timeoutAndCancellationSource = CancellationTokenSource.CreateLinkedTokenSource(
                        cancellationToken, timeoutCts == null ? CancellationToken.None : timeoutCts.Token))
                    {
                        ...
                        {
                            // Execute the operation
                            var result = await operationToAttempt(timeoutAndCancellationToken).ConfigureAwait(false);
                            ...
                            if (errorDetectionStrategy.IsFailure(result))
                            {
                                ...
                                if (notificationPolicy != null)
                                {
                                    await notificationPolicy.OnErrorConditionAsync(shouldRetry, result, cancellationToken);
                                }
